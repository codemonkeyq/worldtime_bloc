...
class ClockPainter extends CustomPainter {
  ...
  void paint(Canvas canvas, Size size) {
    ...
    final hourseP1 = center;
    double houseDegree = 360 / 12 * (time.hour - 12);
    x = (size.width / 2) +
        (size.width / 3 - 50) * cos(Vector.radians(houseDegree));
    y = (size.width / 2) +
        (size.width / 3 - 50) * sin(Vector.radians(houseDegree));

    final hoursP2 = Offset(x, y);
    paint.color = Colors.white;
    paint.strokeWidth = 4;
    canvas.drawLine(hourseP1, hoursP2, paint);
